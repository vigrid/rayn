...
pub fn estimate_normal(scene: fn(Vector3<f32>) -> f32, position: Vector3<f32>, epsilon: f32) -> Vector3<f32> {
    ...
    let y1 = scene(Vector3 { x: position.y + epsilon, ..position });
    let y0 = scene(Vector3 { x: position.y - epsilon, ..position });
    let z1 = scene(Vector3 { x: position.z + epsilon, ..position });
    let z0 = scene(Vector3 { x: position.z - epsilon, ..position });
